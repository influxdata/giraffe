...
- **xDomain**: _array[min, max]. Optional. When used, must also include at least 2 additional properties. 1) Must also include **onSetXDomainDefault** or **onSetXDomain**, or both. 2) Must also include **onResetXDomainDefault** or **onResetXDomain**, or both._ The x domain of the plot can be explicitly set with numbers denoting a minimum and a maximum value for the x-axis. If this option is passed, both _min_ and _max_ are required to be numbers, making the `<Plot>` operate in a "controlled" mode, where it always uses the passed x domain. Any brush interaction with the `<Plot>` that should change the x domain will use Giraffe's built in default handlers if `onSetXDomainDefault` is `true`. Additionally, the `onSetXDomain` callback function will be invoked if provided. Double clicking the plot will call Giraffe's built in default handers if `onResetXDomainDefault` is `true`. Additionally, the `onResetXDomain` callback function will be invoked if provided. If the `xDomain` option is not passed, then the component is "uncontrolled". It will compute, set, and reset the `xDomain` automatically.

- **onSetXDomainDefault**: _boolean. Optional, for use with **xDomain**._ See above regarding **xDomain**.

- **onSetXDomain**: _function(array[min, max]). Optional, for use with **xDomain**._ See above regarding **xDomain**.

- **onResetXDomainDefault**: _boolean. Optional, for use with **xDomain**._ See above regarding **xDomain**.

- **onResetXDomain**: _function(). Optional, for use with **xDomain**._ See above regarding **xDomain**.

- **yDomain**: _array[min, max]. Optional. When used, must also include at least 2 additional properties. 1) Must also include **onSetYDomainDefault** or **onSetYDomain**, or both. 2) Must also include **onResetYDomainDefault** or **onResetYDomain**, or both._ The y domain of the plot can be explicitly set with numbers denoting a minimum and a maximum value for the y-axis. If this option is passed, both _min_ and _max_ are required to be numbers, making the `<Plot>` operate in a "controlled" mode, where it always uses the passed y domain. Any brush interaction with the `<Plot>` that should change the y domain will use Giraffe's built in default handlers if `onSetYDomainDefault` is `true`. Additionally, the `onSetYDomain` callback function will be invoked if provided. Double clicking the plot will call Giraffe's built in default handers if `onResetYDomainDefault` is `true`. Additionally, the `onResetYDomain` callback function will be invoked if provided. If the `yDomain` option is not passed, then the component is "uncontrolled". It will compute, set, and reset the `yDomain` automatically.

- **onSetYDomainDefault**: _boolean. Optional, for use with **yDomain**._ See above regarding **yDomain**.

- **onSetYDomain**: _function(array[min, max]). Optional, for use with **yDomain**._ See above regarding **yDomain**.

- **onResetYDomainDefault**: _boolean. Optional, for use with **yDomain**._ See above regarding **yDomain**.
